...
func GetAllComment(page, limit int64, filter map[string]string) (lists []*CommentList, total int64) {
	offset := (page - 1) * limit
...
func (comment *Comment) Pub() (id int64, err error) {
	comment.Posttime = time.Now()
	id, err = orm.NewOrm().Insert(comment)
	return id, err
}
