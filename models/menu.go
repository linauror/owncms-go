...
type Menu struct {
	...
	Type     string `orm:"column(type);size(10)" description:"菜单类型"`
	Nav      string `orm:"column(nav);size(50)" description:"菜单"`
	Url      string `orm:"column(url);size(50)" description:"链接"`
	Target   string `orm:"column(target);size(10)" description:"打开方式"`
	Reid     int8   `orm:"column(reid)" description:"父级ID"`
	Sortrank int8   `orm:"column(sortrank)" description:"排序"`
}
...
func MenuLists() ([]*Menu, int64) {
	query := orm.NewOrm().QueryTable(new(Menu))
	total, _ := query.Count()
	list := make([]*Menu, 0)
	query.OrderBy("-sortrank").All(&list)
